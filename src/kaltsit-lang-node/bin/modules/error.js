...
var throwError = function (pointer, errorCode) {
    var currentError = ERROR_CODE[errorCode] ? ERROR_CODE[errorCode] : ERROR_CODE.UNKNOWN_ERROR;
    return new Error("Error occured on line ".concat(pointer, ": ").concat(ERROR_STRINGS[Math.round(Math.random() * (ERROR_STRINGS.length - 1))], "\n\nPRTS: ").concat(currentError));
};
